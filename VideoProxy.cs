    ...
    class YouTubeProxy
    {
        ...
        private static async Task<Result> CustomImportAsync(Slot __instance, Slot slot, ImportItem item, Button proxyButton)
        {
            ...
            try {
                ...
                {
                    Msg($"Attempting to process video URL: {item.filePath}");
                    Match match = Regex.Match(item.filePath.ToString(), youtubeIdRegex, RegexOptions.Compiled | RegexOptions.IgnoreCase);

                    if (match.Success) 
                    {
                        id = match.Groups[3].Value;
                        Msg($"Found video id: {id}");
                    }
